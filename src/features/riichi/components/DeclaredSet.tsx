    ...
    render() {
        if (position.next(this.props.current) === this.props.set.payload.from) {
            return <View style={styles.container} >
                {this.props.set.payload.tails.map((t, i) => tileImage(t, i))}
            ...
            return <View style={styles.container}>
                {tileImage(this.props.set.payload.tails[0]!, 0)}
                {this.horizontalTile(this.props.set.payload.claimedTile, 1)}
                {tileImage(this.props.set.payload.tails[1]!, 2)}
            </View>
        }
        return <View style={styles.container}>
            {this.horizontalTile(this.props.set.payload.claimedTile, 0)}
            {tileImage(this.props.set.payload.tails[0]!, 1)}
            {tileImage(this.props.set.payload.tails[1]!, 2)}
        </View>
    }
};